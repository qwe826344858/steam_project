...
class Logger:
    ...
    def init():
        ...
        current_file_name = os.path.basename(log_file_name)
        Logger.save_path = f"{Logger.log_config['save_path']}/{datetime.datetime.now().date()}/{current_file_name}.log"
    ...
    def info(cls, message):
        ...
        log_message += f" | path:{cls.log_file_path} Called from {caller_info.filename}, {caller_info.function}(), Line {caller_info.lineno}"

        print(f"写日志 message:{message}")
        asyncio.run(cls.process_info(log_message))
